...
def _names_match(stored: str, lookup: str) -> bool:
    ...
    # Case 3: fuzzy match for transliteration differences (k/c, i/y, etc.)
    if SequenceMatcher(None, sc, lc).ratio() >= 0.82:
        return True
...
def _get_prediction_info(tennis_conn: sqlite3.Connection, event_id: str, player_name: str):
    ...
    if _names_match(_norm(p1), norm_p):
        return (elo1, elo2, p2, mp1, fp1)
    if _names_match(_norm(p2), norm_p):
