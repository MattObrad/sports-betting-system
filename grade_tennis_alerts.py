...
def _name_matches(a: str, b: str) -> bool:
    """True if two already-normalized names are equal or close (SequenceMatcher >= 0.82).
    Catches one-letter transliteration differences like k/c, i/y that accent-strip misses."""
    if a == b:
        return True
...
def _get_opponent(tennis_conn: sqlite3.Connection, game_id: str, player_name: str) -> str:
    ...
    if _name_matches(_norm(p1), norm_p):
        return p2
    if _name_matches(_norm(p2), norm_p):
        return p1
    return ""
...
def find_result(
    ...
    for winner, loser in rows:
        w_norm = _norm(winner)
        l_norm = _norm(loser)
        if _name_matches(w_norm, norm_player):
            if norm_opp and not _name_matches(l_norm, norm_opp):
                continue   # right player, wrong match
            return "WIN"
        if _name_matches(l_norm, norm_player):
            if norm_opp and not _name_matches(w_norm, norm_opp):
                continue   # right player, wrong match
            return "LOSS"
    return None
