...
# display_name is stored in games.league — must be <= 10 chars.
SPORT_ENDPOINTS = [
    # American Football
...
def classify_prop_market(criterion_label):
    """Map a Player Occurrence Line criterion label to a clean market_type."""
...
def process_props_for_event(cur, event_id):
    ...
    of every betOfferType.name encountered (used for console discovery logging).
...
def collect_sport(conn, sport_name, endpoint, sport_key):
    ...
    if all_offer_types:
        print(f"  betOfferType.name values seen for {sport_name}:")
        for t in sorted(all_offer_types):
            print(f"    - {t}")
    print()
...
def main():
    ...
    try:
        ...
        for sport_name, endpoint, sport_key in SPORT_ENDPOINTS:
            collect_sport(conn, sport_name, endpoint, sport_key)
    finally:
        conn.close()
